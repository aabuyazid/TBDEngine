...
void VulkanEngine::init_swapchain() {
    ...
    // Draw image size will match the window; TODO for gui
    VkExtent3D drawImageExtent = {
        _windowsExtent.width,
        _windowsExtent.height,
        1
    };
    ...
    VkImageCreateInfo rimg_info = 
        vkinit::image_create_info(_drawImage.imageFormat, 
                                  drawImageUsages, drawImageExtent);

    // Allocate GPU local memory for draw image
    ...
    // Creating view for the draw image to use
    VkImageViewCreateInfo rview_info = 
        vkinit::imageview_create_info(_drawImage.imageFormat, 
                _drawImage.image, VK_IMAGE_ASPECT_COLOR_BIT);

    VK_CHECK( vkCreateImageView(_device, &rview_info, 
                nullptr, &_drawImage.imageView) );
...
void VulkanEngine::init_descriptors() {
    ...
    _drawImageDescriptors = globalDescriptorAllocator.allocate(_device, 
            _drawImageDescriptorLayout);
...
void VulkanEngine::draw_background(VkCommandBuffer cmd) {
    ...
    // Bind the descriptor set containing draw image
    vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_COMPUTE, 
        _gradientPipelineLayout, 0, 1, &_drawImageDescriptors, 0, nullptr);

    // Execute compute pipeline with 16x16 workgroup
